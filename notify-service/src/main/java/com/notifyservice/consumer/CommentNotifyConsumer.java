...
public class CommentNotifyConsumer {
    ...
    public void handleCommentNotify(CommentNotifyMessage message, Message rawMessage, Channel channel,
     ...
     */
    private void rejectOrDeadLetter(CommentNotifyMessage message, Message rawMessage, Channel channel,
                                    long deliveryTag) throws IOException {
        if (getRetryCount(rawMessage) >= MAX_RETRY_COUNT) {
            rabbitTemplate.convertAndSend(
                    MqConstants.COMMENT_NOTIFY_DLX,
                    MqConstants.COMMENT_NOTIFY_DLQ_ROUTING_KEY,
                    message);
            channel.basicAck(deliveryTag, false);
            log.warn("评论通知超过最大重试次数，已进入DLQ, receiverId={}", message.getReceiverId());
            return;
        }
